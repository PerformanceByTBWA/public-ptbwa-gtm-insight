...
- name: '[PTBWA] gclid is captured from URL and stored in cookie'
  code: |-
    var mockUrl = [];
    ...
    assertThat(mockUrl[0]).contains('gclid=EAIaIQobChMI_test_gclid_value');
- name: '[PTBWA] gclid is read from cookie when not present in URL'
  code: |-
    var mockUrl = [];
    mock('sendPixel', function(url, onSuccess) {
      mockUrl.push(url);
    ...
    assertThat(mockUrl[0]).contains('gclid=EAIaIQobChMI_stored_gclid');
- name: '[PTBWA] empty gclid is sent when not present anywhere'
  code: |-
    var mockUrl = [];
    mock('sendPixel', function(url, onSuccess) {
      mockUrl.push(url);
    ...
    // URL and Cookies are empty (setup handles empty defaults)
...
- name: '[PTBWA] Numeric zero conversion value is preserved'
  code: |-
    ...
    assertThat(mockUrl[0]).contains('conv_value=0');
    assertApi('setCookie').wasCalledWith('_insight_uid', 'test-cid-1111', {
      'max-age': 60 * 60 * 24 * 30,
      'secure': true,
      'path': '/'
    });
- name: '[PTBWA] Current URL UTM parameters are serialized and sent'
  code: |-
    var mockUrl = [];
    mock('sendPixel', function(url, onSuccess) {
      mockUrl.push(url);
    ...
    assertThat(mockUrl[0]).contains('utm_source=google');
    assertThat(mockUrl[0]).contains('utm_medium=cpc');
    assertThat(mockUrl[0]).contains('utm_campaign=summer_sale');
    assertThat(mockUrl[0]).contains('utm_term=running%20shoes');
    assertThat(mockUrl[0]).contains('utm_content=hero_banner');
